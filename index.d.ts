export class MdnsDiscovery extends TypedEmitter<MdnsDiscoveryEvents> {
  constructor()
  lookup(
    serviceType: import('@gravitysoftware/dnssd').ServiceIdentifier | string
  ): Promise<void>
  stopLookup(): Promise<void>
  announce(
    serviceType: import('@gravitysoftware/dnssd').ServiceIdentifier | string,
    options: import('@gravitysoftware/dnssd').Service
  ): void
...
export type MdnsDiscoveryEvents = {
  ...
  serviceChanged: (
    serviceChanged: import('@gravitysoftware/dnssd').Service
  ) => void
